...
The Lua SDK for the WaterQualityArchive API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Measurement(nil):list(nil, nil)
...
local result, err = client:WaterQualityArchive(nil):load(
  { id = "test01" }, nil
)
