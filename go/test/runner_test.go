...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("WATERQUALITYARCHIVE_TEST_LIVE") == "TRUE" ||
		os.Getenv("WATERQUALITYARCHIVE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("WATERQUALITYARCHIVE_TEST_EXPLAIN"); explain != "" {
