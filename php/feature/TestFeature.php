...
// WaterQualityArchive SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class WaterQualityArchiveTestFeature extends WaterQualityArchiveBaseFeature
{
    ...
    public function init(WaterQualityArchiveContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (WaterQualityArchiveContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = WaterQualityArchiveParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (WaterQualityArchiveContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(WaterQualityArchiveContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = WaterQualityArchiveParam::call($ctx, $k);
