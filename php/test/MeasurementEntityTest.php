...
class MeasurementEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = WaterQualityArchiveSDK::test($seed, null);
        $seen = iterator_to_array($base->Measurement(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = WaterQualityArchiveConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = WaterQualityArchiveSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set WATER_QUALITY_ARCHIVE_TEST_MEASUREMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function measurement_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("WATER_QUALITY_ARCHIVE_TEST_MEASUREMENT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["WATER_QUALITY_ARCHIVE_TEST_MEASUREMENT_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["WATER_QUALITY_ARCHIVE_TEST_LIVE"] === "TRUE") {
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["WATER_QUALITY_ARCHIVE_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
