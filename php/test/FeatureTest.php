...
// WaterQualityArchive SDK feature test
//
...
class FtCtrl extends WaterQualityArchiveControl
{
...
class FtClient
{
    ...
    private WaterQualityArchiveUtility $utility;

    public function __construct(WaterQualityArchiveUtility $utility, array $options)
    {
    ...
    public function get_utility(): WaterQualityArchiveUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WaterQualityArchiveContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WaterQualityArchiveContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new WaterQualityArchiveUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (WaterQualityArchiveContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WaterQualityArchiveConfig::make_feature($name);
    ...
    private static function buildUrl(WaterQualityArchiveSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WaterQualityArchiveError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WaterQualityArchiveResponse($fetched);
        }
    ...
    private function fail(WaterQualityArchiveContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WaterQualityArchiveContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
