...
//     (\`new WaterQualityArchiveSDK(...)\` / \`WaterQualityArchiveSDK.test(...)\` become
//     \`WaterQualityArchiveSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WaterQualityArchiveSDK'
...
const TEST_SEED = {"entity":{"measurement":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WaterQualityArchiveSDK(...)\` and any
// \`WaterQualityArchiveSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WaterQualityArchiveSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WaterQualityArchiveSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
