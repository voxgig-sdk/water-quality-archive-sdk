...
local function make_error_util(ctx, err)
  ...
  local msg = "WaterQualityArchiveSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = WaterQualityArchiveError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == WaterQualityArchiveError then
