...
// WaterQualityArchive SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        WaterQualityArchiveSDK $client,
        WaterQualityArchiveUtility $utility
    ): WaterQualityArchiveContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new WaterQualityArchiveContext($ctxmap, null);
    ...
    private static function fixctx(WaterQualityArchiveContext $ctx, WaterQualityArchiveSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?WaterQualityArchiveError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new WaterQualityArchiveError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new WaterQualityArchiveResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new WaterQualityArchiveBaseFeature();
    ...
    {
        $hook_client = WaterQualityArchiveSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends WaterQualityArchiveBaseFeature {
            public $hook_fn = null;
            public function TestHook(WaterQualityArchiveContext $ctx): void
            {
    ...
    {
        $init_client = WaterQualityArchiveSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends WaterQualityArchiveBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new WaterQualityArchiveSDK([
            'system' => [
    ...
    {
        $blocked_client = new WaterQualityArchiveSDK([
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new WaterQualityArchiveSpec([
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new WaterQualityArchiveSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = WaterQualityArchiveSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new WaterQualityArchiveOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = WaterQualityArchiveSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof WaterQualityArchiveError) ? $result->err->msg : (string)$result->err;
