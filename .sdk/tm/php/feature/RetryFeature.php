...
// WaterQualityArchive SDK retry feature
...
class WaterQualityArchiveRetryFeature extends WaterQualityArchiveBaseFeature
{
    ...
    public function init(WaterQualityArchiveContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WaterQualityArchiveContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WaterQualityArchiveContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WaterQualityArchiveContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
