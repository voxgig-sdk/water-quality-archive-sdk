# WaterQualityArchive SDK test runner
...
class WaterQualityArchiveTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WaterQualityArchiveTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WaterQualityArchiveTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WaterQualityArchiveTestRunner.getenv("WATER_QUALITY_ARCHIVE_TEST_LIVE")
        override = WaterQualityArchiveTestRunner.getenv("WATER_QUALITY_ARCHIVE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WaterQualityArchiveTestRunner.getenv(key)
        ...
        explain = WaterQualityArchiveTestRunner.getenv("WATER_QUALITY_ARCHIVE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WaterQualityArchiveTestRunner._test_control is not None:
            return WaterQualityArchiveTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WaterQualityArchiveTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WaterQualityArchiveTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WaterQualityArchiveTestRunner.load_test_control()
...
def load_env_local():
    WaterQualityArchiveTestRunner.load_env_local()


def env_override(m):
    return WaterQualityArchiveTestRunner.env_override(m)


def entity_data(v):
    return WaterQualityArchiveTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WaterQualityArchiveTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WaterQualityArchiveTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WaterQualityArchiveTestRunner.load_test_control()


def live_delay_ms():
    return WaterQualityArchiveTestRunner.live_delay_ms()
