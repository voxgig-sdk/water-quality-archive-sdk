...
class TestMeasurementEntity:
    ...
    def test_should_stream(self):
        ...
        base = WaterQualityArchiveSDK.test(seed, None)
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = WaterQualityArchiveSDK.test(
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set WATER_QUALITY_ARCHIVE_TEST_MEASUREMENT_ENTID JSON to run live")
        client = setup["client"]
...
def _measurement_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("WATER_QUALITY_ARCHIVE_TEST_MEASUREMENT_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("WATER_QUALITY_ARCHIVE_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("WATER_QUALITY_ARCHIVE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("WATER_QUALITY_ARCHIVE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
