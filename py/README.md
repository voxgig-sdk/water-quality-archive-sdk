...
The Python SDK for the WaterQualityArchive API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Measurement(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.WaterQualityArchive(None).load(
    {"id": "test01"}, None
)
