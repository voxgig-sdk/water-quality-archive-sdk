...
require_relative 'WaterQualityArchive_types'
...
class WaterQualityArchiveSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WaterQualityArchiveHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WaterQualityArchiveHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WaterQualityArchiveFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WaterQualityArchiveUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WaterQualityArchiveHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WaterQualityArchiveHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WaterQualityArchiveHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WaterQualityArchiveError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue WaterQualityArchiveError => err
    ...
    if fetched.is_a?(Hash)
      status = WaterQualityArchiveHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WaterQualityArchiveError.new(
        "graphql_error", "WaterQualityArchiveSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WaterQualityArchiveSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
