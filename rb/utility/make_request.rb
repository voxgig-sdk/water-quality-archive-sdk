...
module WaterQualityArchiveUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WaterQualityArchiveResponse.new({})
    ...
    if fetch_err
      ...
      response = WaterQualityArchiveResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WaterQualityArchiveResponse.new(fetched)
    else
