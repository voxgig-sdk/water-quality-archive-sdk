# water-quality-archive-mcp

[MCP](https://modelcontextprotocol.io) server exposing the WaterQualityArchive SDK as
two agent tools — `water-quality-archive_list` and `water-quality-archive_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/water-quality-archive-mcp)
make build

# 2. Provide credentials via the environment
export WATER_QUALITY_ARCHIVE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user water-quality-archive \
  -- /absolute/path/to/water-quality-archive-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./water-quality-archive-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// water-quality-archive_list: first page of records
{ "entity": "measurement" }
{ "entity": "measurement", "query": { } }

// water-quality-archive_load: one record by id
{ "entity": "measurement", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/water-quality-archive-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user water-quality-archive \
     -- "$PWD"/dist/*/water-quality-archive-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `water-quality-archive_list` and `water-quality-archive_load` tools now appear
   in new sessions. Ask the agent to *"list measurement using water-quality-archive"*
   and it calls `water-quality-archive_list` with `{"entity":"measurement"}`.
...
```sh
export WATER_QUALITY_ARCHIVE_APIKEY=sk_live_xxx            # API key
export WATER_QUALITY_ARCHIVE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `water-quality-archive_list` tool
...
### Call the `water-quality-archive_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `water-quality-archive_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `water-quality-archive_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WATER_QUALITY_ARCHIVE_APIKEY` | API key sent with every request. |
| `WATER_QUALITY_ARCHIVE_BASE` | Optional override of the API base URL. |
...
```sh
./water-quality-archive-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"water-quality-archive_load","arguments":{"entity":"measurement","query":{"id":1}}}}'
```
