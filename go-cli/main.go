// Command WaterQualityArchive-cli is a CLI + REPL for the WaterQualityArchive SDK,
...
const prompt = "water-quality-archive"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "measurement"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WATER_QUALITY_ARCHIVE_APIKEY (api key), WATER_QUALITY_ARCHIVE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WATER_QUALITY_ARCHIVE_APIKEY carries the API key and
	// WATER_QUALITY_ARCHIVE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WATER_QUALITY_ARCHIVE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WATER_QUALITY_ARCHIVE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWaterQualityArchiveSDK(opts)
