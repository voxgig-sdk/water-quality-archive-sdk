...
boru-driven command-line client **and** interactive REPL for the WaterQualityArchive
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/water-quality-archive-cli)
make build

# 2. See usage (words, entities, env vars)
./water-quality-archive-cli --help
...
WATER_QUALITY_ARCHIVE_BASE=https://api.example.com ./water-quality-archive-cli list measurement

# 6. No arguments -> interactive REPL
./water-quality-archive-cli
water-quality-archive> list measurement
water-quality-archive> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/water-quality-archive-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/water-quality-archive-cli list measurement
   ```
...
Run with no arguments to open a REPL (prompt `water-quality-archive>`). Each line is
evaluated as its own boru expression:

```text
$ ./water-quality-archive-cli
water-quality-archive> list measurement
water-quality-archive> /help
water-quality-archive> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `measurement`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/water-quality-archive-cli`. |
...
(`./water-quality-archive-cli <expr>`) and interactively (the REPL), and expressions compose the
